...
class SelfApplyController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        try {
            ...
            if (!$user || (Cookie::has('user_mobile') && Cookie::get('user_mobile') != $inputs['mobile'])) {
                $keysToKeep = ['XSRF-TOKEN', 'lendingsathi_session', 'utm_campaign', 'utm_medium', 'utm_source'];
            ...
            if ($txStatus == "PAYMENT_SUCCESS") {
                ...
                if ($isEntry == 0) {
                    ...
                    if (!$existingInvoice) {
                        ...
                        /* send email in brevo */
                        sendBrevoHtmlMail2($mailData, 'Congratulations! Payment Successful for LendingSathi Self-Apply Plan.', $sendGreetings, 3, $attachments);
                    }
                    if ($response2 > 0) {
            ...
            if (isset($loanType, $applyId, $orderId) && $loanType !== null && $applyId !== null && $orderId !== null) {
                ...
                if (isset($responsecode) && $responsecode == 100) {
                    ...
                    $fbdata = array(
                        ...
                        // 'sourceurl' => 'https://lendingsathi.com/self-apply/paymentSuccess'
    ...
    public function getOffer1(Request $request)
    {
        try {
            ...
            // $returnUrl = 'https://lendingsathi.com/api/self-apply/prime-offer-response';
    ...
    public function getOffer2_easebuzz(Request $request)
    {
        try {
            $inputs = $request->all();
            ...
            if (in_array($mobile, $uatNumbers)) {
                $grandAmount = 1;
            }

            /* Insert cardoffer */
            DB::table('cardoffer')->updateOrInsert(
                ['mobile' => $mobile],
                [
                    'rec_date'   => now(),
                    'offerpage'  => 5,
            ...
            $txnid = time() . rand(100, 999);

            /* Save order */
            EasebuzzEntry::insert([
                'rec_date'    => now(),
                'entryfor'    => 7,
                'userid'      => $offerId,
                'orderid'     => $txnid,
            ...
            $success_url = route('api.self.apply.easebuzz-response');
            $failure_url = route('api.self.apply.easebuzz-response');

            /* HASH GENERATE */
            $hash_string = $key . '|' . $txnid . '|' . round($grandAmount) . '|' . $productinfo . '|' . $name . '|' . $email . '|||||||||||' . $salt;
            $hash = strtolower(hash('sha512', $hash_string));

            $payment_url = (env('EASEBUZZ_ENV') == 'PROD')
                ? "https://pay.easebuzz.in/payment/initiateLink"
                : "https://testpay.easebuzz.in/payment/initiateLink";

            $easebuzz_env = env('EASEBUZZ_ENV'); // TEST or PROD
            ...
            $postData = [
                ...
                'productinfo' => $products->productname,
                'orderid'     => $offerId,
                'surl'        => route('api.self.apply.easebuzz-response'),
                'furl'        => route('api.self.apply.easebuzz-response'),
            ];

            /* INITIATE PAYMENT */
            $payment = new Payment();
            
            $response = $payment->initiate_payment(
                $postData,
                $key,
                $salt,
                $easebuzz_env
    ...
    public function offer2Response_easebuzz(Request $request)
    {
        try {
            $meta = selfApplyMeta();
            ...
            $txnid  = $request->txnid;
            $easebuzzId = $request->easepayid;

            $paymentData = EasebuzzEntry::where('orderid', $txnid)->first();

            if (!$paymentData) {
                return view('cardoffer-response', ['meta' => $meta, 'response' => false]);
            }

            EasebuzzEntry::where('id', $paymentData->id)->update([
                'transactionid' => $easebuzzId,
                'txstatus'    => $status,
            ...
            if ($status == 'success') {
                ...
                Cardoffer::where('id', $paymentData->userid)->update([
                    'rec_date' => now(),
                    'card_number' => $cardno,
                    'registration_date' => date('Y-m-d'),
                    'expiry_date' => date('Y-m-d', strtotime('+3 months')),
                    'paymentid' => $easebuzzId,
                ...
                if ($regUser) {
                    convertIntoCustomer($cardno, $regUser, $userData, $paymentData->orderamount, $easebuzzId, 1, 'self-apply', 'SA_', 5);
                } else {
                    sendPaymentGreetings(
    ...
    public function getOffer3_cipherpay(Request $request)
    {
        try {
            ...
            $orderId = number_format(microtime(true) * 1000, 0, '.', '');
            $encData = null;
            // $returnUrl = 'https://lendingsathi.com/api/self-apply/premium-offer-response';
            ...
            $request = array(
                ...
                "parameter" => [
                    ...
                    'receiver_vpa' => "cpy.lendingsathi@finobank",
    ...
    public function getOffer4_lyra(Request $request)
    {
        try {
            $inputs = $request->all();
            $request->validate([
            ...
            // $returnUrl = 'https://lendingsathi.com/api/self-apply/star-offer-response';
    ...
    public function offer4Response_lyra(Request $request)
    {
        try {
            $inputs = $request->all();
            //Log::info('request data - '.json_encode($request->all()));
    ...
    public function getOffer4_easebuzz(Request $request)
    {
        try {
            $inputs = $request->all();
            ...
            $success_url = route('api.self.apply.offer4Response');
            $failure_url = route('api.self.apply.offer4Response');
            ...
            $postData = [
                ...
                'productinfo' => $products->productname,
                'orderid'     => $offerId,
                'surl'        => route('api.self.apply.offer4Response'),
                'furl'        => route('api.self.apply.offer4Response'),
            ];
            ...
            log::info("response payment_url" . $response['payment_url']);
            return response()->json([
                'type' => $response['status'] == 1 ? 'SUCCESS' : 'ERROR',
                'payment_url' => $response['payment_url'] ?? null,
                'message' => $response['message'] ?? null
    ...
    public function offer4Response_easebuzz(Request $request)
    {
        try {
            $meta = selfApplyMeta();
